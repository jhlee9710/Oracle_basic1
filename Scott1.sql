...
create or replace procedure emp01_pro
is
    vempno number(10);
    vename varchar2(10);
begin
...
--===============================================
create or replace procedure del_empno(vempno emp01.empno%type)
is
...
exec sal_empno(7521,:var_ename,:var_sal,:var_job);

print var_ename;
print var_sal;
print var_job;

-- 사원 정보를 저장하는 저장 프로시저 만들기
-- 사번, 이름, 직책, 매니저, 부서
-- 사원정보는 매개변수를 사용해서 받아온다.
create table emp02
as
...
create or replace procedure info_empno(
...
/
exec info_empno(1111,'hong','sales',2222,10);
...
begin
    open 커서명;
    loop
        fectch 커서명 into 변수명; -- 테이블로부터 가져와서 변수에 저장하는역할
...
begin
    open c1;
    loop
        fetch c1 into vemp; -- 테이블로부터 가져와서 변수에 저장하는역할
        exit when c1%notfound;
        dbms_output.put_line(vemp.empno || ' ' || vemp.ename || ' ' || vemp.job || ' ' || vemp.mgr || ' ' || vemp.sal || ' ' || vemp.comm || ' ' || vemp.deptno);
    end loop;
    close c1;
end;
/
